...
const loginController = async (req, res) => {
  try {
    ...
    const refreshToken = await JWT.sign({_id:user._id}, process.env.JWT_SECRET,{
      expiresIn:"4h"
    })
...
const sendResetPasswordMail = async(name,email,otp)=>{
  ...
  const transporter = nodemailer.createTransport({
    ...
    const mailOptions = {
      ...
      html:'<p> Hii '+name+', please copy the link and reset the password . this is your OTP '+otp+'</p>'
    }
...
const forgotPasswordController = async (req,res)=>{
  try{
    ...
    console.log(OtpData);
      ...
      const data = await userModel.updateOne({email:email},{$set:{otp:OtpData}});
      console.log(data);
...
const profileController = async(req,res)=>{
  try{
    ...
    const valueData= await userModel.findById(userData._id).select('name email role');
    res.status(200).send({
      success:true,
...
const profileUpdateController = async (req,res)=>{
  try{
    ...
    if (name || email || phone || address) {
      const userData = await  userModel.findByIdAndUpdate(req.user._id,{$set:{
...
module.exports = { registerController, loginController, testController,forgotPasswordController,resetPasswordController,profileController,profileUpdateController};
